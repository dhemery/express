...
import com.dhemery.express.DiagnosingPredicate;
...
public class NamedPredicateTests {
    ...
    public void delegatesTestToTheUnderlyingPredicate() {
        Predicate<String> startsWithF = new DiagnosingPredicate<>(IGNORED_DESCRIPTION, s -> s.startsWith("f"));
    ...
    public void describesItselfWithTheGivenName() {
        ...
        Predicate<String> descriptivePredicate = new DiagnosingPredicate<>(name, IGNORED_PREDICATE);
    ...
    public void or_yieldsAPredicateThatPerformsALogical_OR_OfThisPredicateAndAnother() {
        Predicate<String> startsWithF = new DiagnosingPredicate<>(IGNORED_DESCRIPTION, s -> s.startsWith("f"));
        Predicate<String> endsWithO = new DiagnosingPredicate<>(IGNORED_DESCRIPTION, s -> s.endsWith("o"));
    ...
    public void or_yieldsAPredicateNamedToDescribeItsComposition() {
        Predicate<String> first = new DiagnosingPredicate<>("first", IGNORED_PREDICATE);
        Predicate<String> second = new DiagnosingPredicate<>("second", IGNORED_PREDICATE);
    ...
    public void negate_yieldsAPredicateNamedToDescribeItsComposition() {
        Predicate<String> startsWithF = new DiagnosingPredicate<>("starts with F", IGNORED_PREDICATE);
